...
class GerenciadorMatriculas:
    ...
    def alterar(self, matricula: Matricula) -> None:
        ...
        matricula.status = matricula.status.strip().capitalize() 
    ...
    def excluir(self, id_matricula: int) -> None:
        """
        Garante regras de segurança histórica antes da exclusão.
        """
    ...
    def _validar_data(self, data_texto: str) -> None:
        """
        Tenta converter a string de data para garantir que não foram digitados 
        valores absurdos (ex: 32/13/2026).
        """
        try:
